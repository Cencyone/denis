...
class DataProvider
{
    ...
    public function getBrands($array = true) {
        ...
        if (!$this->brands) {
            ...
            $this->brands = \SBrandsQuery::create()->select([SBrandsTableMap::COL_ID, SBrandsI18nTableMap::COL_NAME])
    ...
    public function getCategoriesOptions() {
        $categories = \SCategoryQuery::create()
            ->getTree(0, \SCategoryQuery::create()->joinWithI18n(\MY_Controller::defaultLocale()))
            ->getCollection();
    ...
    /**
     * @return array RUB to main currency
     */
    public function getCurrencies() {
        ...
        if (!$this->currencies) {
            ...
            foreach ($dbCurrencies as $currency) {
                $code = $currency->getCode() == 'RUB' ? 'RUR' : $currency->getCode();
                ...
                if ($code == 'RUR') {
                    ...
                    $rubId = $currency->getId();
                } else {
                    $currencies[$currency->getId()] = ['code' => $code, 'rate' => 1 / $currency->getRate()];
                }
    ...
    private function getVariants($categoryIds = null, $brandIds = null) {
        ...
        $query = \SProductVariantsQuery::create()
            ->select(['id', 'url', 'currencyId', 'product_id'])
        ...
        $variants = $query->find()->toArray('id');
